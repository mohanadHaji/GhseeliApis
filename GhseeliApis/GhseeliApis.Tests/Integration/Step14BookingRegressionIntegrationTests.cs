...
public sealed class Step14BookingRegressionIntegrationTests
{
    ...
    [Fact]
    [Trait("ScenarioId", "STEP14-REGRESSION-BOOKING-LIST-067")]
    public async Task LegacyMyBookings_RemainsOneLegacyBookingAndExcludesPaymentAuthority()
    {
        ...
        var handler = new Mock<IBookingHandler>();
        handler.Setup(value => value.GetByUserIdAsync(legacy.UserId))
            .ReturnsAsync([legacy]);
        await using var factory = new CheckoutDraftApiFactory(
            configureTestServices: services =>
            {
                services.RemoveAll<IBookingHandler>();
                services.AddSingleton(handler.Object);
            });
        using var client = factory.CreateApiClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
            "Bearer",
            CreateJwt(legacy.UserId));

        using var response = await client.GetAsync("/api/Bookings/my-bookings");
        var body = await response.Content.ReadAsStringAsync();
        using var document = JsonDocument.Parse(body);

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        document.RootElement.GetArrayLength().Should().Be(1);
        document.RootElement[0].GetProperty("id").GetGuid().Should().Be(legacy.Id);
        document.RootElement[0].EnumerateObject().Select(value => value.Name)
            .Should().NotContain(["customerPayment", "paymentIntentId", "clientSecret",
                "providerPublishableKey", "chargeId"]);
        body.Should().NotContain("CustomerPayment");
        handler.Verify(value => value.GetByUserIdAsync(legacy.UserId), Times.Once);
    }
    ...
    [Trait("ScenarioId", "STEP14-REGRESSION-BOOKING-CANCEL-068")]
    public async Task LegacyCancellation_CannotMutateAuthoritativePaymentOrTriggerProvider(
        ...
        string paymentState)
    {
        ...
        var handler = new Mock<IBookingHandler>();
        handler.Setup(value => value.CancelAsync(legacyId, userId)).ReturnsAsync(true);
        ...
        var authoritative = CreateCustomerBooking();
        authoritative.UserId = userId;
        authoritative.User.Id = userId;
        authoritative.IsPaid = isPaid;
        authoritative.PaymentState = paymentState;
        var payment = CreatePayment(authoritative, paymentStatus);
        await SeedAsync(factory, authoritative, payment);
        var before = await ReadAuthorityAsync(factory, authoritative.Id);
        using var client = factory.CreateApiClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
            "Bearer",
            CreateJwt(userId));
        ...
        using var response = await client.PutAsync(
            $"/api/Bookings/{legacyId:D}/cancel",
            null);

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        (await ReadAuthorityAsync(factory, authoritative.Id)).Should().BeEquivalentTo(before);
