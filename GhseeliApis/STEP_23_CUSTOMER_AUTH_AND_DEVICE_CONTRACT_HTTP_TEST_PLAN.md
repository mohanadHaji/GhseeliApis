...
Status: Done locally - SMTP configuration required before deployment
...
- Execution level: automated TestServer plus mocked SMTP. Live SMTP and
  production execution are excluded until SMTP credentials are configured.
...
- All 19 stable Step 23 scenario IDs passed through automated unit,
...
- Live SMTP delivery is deferred because no SMTP host, credential, or verified
  ...
  SMTP until those values are stored in user/deployment secrets.
